...
void ElementCommandBuffer::recordElement(Shader<Vertex>& shader,
                                         Element& element,
                                         ...
                                         Swapchain& swapchain)
{
    ...
    auto renderPassBeginInfo = vk::RenderPassBeginInfo { shader.getRenderPass(), shader.getFrameBuffers()[index],
    ...
    m_commandBuffer.bindPipeline(vk::PipelineBindPoint::eGraphics, shader.getPipeline());

    vk::Buffer vertexBuffers[] = { shape.getVertexBuffer().getBuffer() };
    vk::DeviceSize offsets[] = {0};

    m_commandBuffer.bindVertexBuffers(0, 1, vertexBuffers, offsets);
    m_commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, shader.getPipelineLayout(), 0, 1, &sets[0], 0, nullptr);
    m_commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, shader.getPipelineLayout(), 1, 1, &sets[1], 0, nullptr);
