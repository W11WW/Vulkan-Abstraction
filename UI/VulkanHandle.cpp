...
void VulkanHandle::initialize(GLFWwindow* window)
{
    ...
    test2.setPosition(glm::vec3(3, 1.5, 1.1));
}
...
void VulkanHandle::updateCommandBuffers()
{
    ...
    std::vector<Element> elements = { test, test2 };

    for(int i = 0; i < m_renderer.getSwapchain().getSize(); i++)
    {
        m_renderer.getCommandBuffers()[i].beginCommandBufferRecord();
        m_renderer.getCommandBuffers()[i].recordElement(m_pipeline, m_renderPass, elements, sets, m_square, i, m_renderer.getSwapchain(), m_renderer.getPhysicalDevice());
